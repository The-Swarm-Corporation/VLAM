...
class TestPerformance:
    ...
    def test_inference_speed_sequences(self, small_model):
        ...
        for i in range(1, len(times)):
            ...
            assert ratio < expected_ratio * 2, f"Scaling not linear: {ratio} vs {expected_ratio}"
    ...
    def test_memory_usage_scaling(self, small_model):
        ...
        for i in range(1, len(memory_usage)):
            if memory_usage[0] > 0:  # Avoid division by zero
                ...
                assert ratio < expected_ratio * 3, f"Memory scaling issue: {ratio} vs {expected_ratio}"
    ...
    def test_gradient_computation_speed(self, small_model):
        ...
        language_loss = torch.nn.functional.cross_entropy(
            outputs['language_logits'].view(-1, 1000),
            target_language.view(-1)
        )
    ...
    def test_gpu_acceleration(self, small_model):
        ...
        assert gpu_time < cpu_time * 2, f"GPU not accelerating: CPU={cpu_time:.3f}s, GPU={gpu_time:.3f}s"
...
class TestMemoryEfficiency:
    ...
    def test_vision_encoder_freezing_memory(self):
        ...
        loss_frozen = torch.nn.functional.mse_loss(outputs_frozen['actions'], target_actions)
